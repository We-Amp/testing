...
def main():
    ...
    stream_id = http2_connection.get_next_available_stream_id()
    print("stream_id: " + str(stream_id))

    http2_connection.send_headers(
        stream_id=stream_id,
        headers=[
            (':method', 'GET'),
            (':path', '/'),
    ...
    if data_to_send:
        tls_connection.sendall(data_to_send)

    print("waiting for server")

    while True:
        data = tls_connection.recv(65535)
